...
public class King extends Piece {
	...
	public ArrayList<Location> calculatePossibleMoves(int a, int b) {
		...
		one.x = x+1;
		one.y = yCurrentPlace;
		if (one.x<8 && one.x>=0 && one.y<8 && one.y>=0) {
		...
		two.x = x-1;
		two.y = yCurrentPlace;
		if (two.x<8 && two.x>=0 && two.y<8 && two.y>=0) {
		...
		three.x = xCurrentPlace;
		three.y = y+1;
		if (three.x<8 && three.x>=0 && three.y<8 && three.y>=0) {
		...
		four.x = xCurrentPlace;
		four.y = y-1;
		if (four.x<8 && four.x>=0 && four.y<8 && four.y>=0) {
		...
		five.x = x+1;
		five.y = y+1;
		if (five.x<8 && five.x>=0 && five.y<8 && five.y>=0) {
		...
		six.x = x+1;
		six.y = y-1;
		if (six.x<8 && six.x>=0 && six.y<8 && six.y>=0) {
		...
		seven.x = x-1;
		seven.y = y-1;
		if (seven.x<8 && seven.x>=0 && seven.y<8 && seven.y>=0) {
		...
		eight.x = x-1;
		eight.y = y+1;
		if (eight.x<8 && eight.x>=0 && eight.y<8 && eight.y>=0) {
	...
	public King(int a, int b, String o) {
		super(a, b, "K ", "king", o);
	}
